...
def stats_action_feature(action_data, n_from, n_to, action_type, action_file):
    ...
    for i in range(1, n):
        ...
        angles.append(angle)
        directions.append(__get_direction(angle))
    ...
    # count action type
    # hist_action_type = defaultdict(int)
    # if action_type is None:
    #     for item in action_data:
    #         hist_action_type[item['state']] += 1
    # for k, v in hist_action_type:
    #     hist_action_type[k] = v / len(action_data)
    # action_type = []
    # for key in ["Move", "Pressed", "Released", "Scroll"]:
    #     action_type.append(hist_action_type[key])
    ACTION_CODE = {
        "Drag": 1,
        "Move": 1,
        "Scroll": 2,
    ...
    if action_type is None:
        action_type = []
        for item in action_data:
            if item["button"] == "Scroll":
                action_type.append(ACTION_CODE[item["button"]])
            else:
                action_type.append(ACTION_CODE[item["state"]])
...
def __get_direction(theta):
    ...
    directions = {
        "1": [0, math.pi/4],
        "2": [math.pi * 1/4, math.pi * 1/2],
        "3": [math.pi * 1/2, math.pi * 3/4],
        "4": [math.pi * 3/4, math.pi],
        "5": [-math.pi, -math.pi * 3/4],
        "6": [-math.pi * 3/4, -math.pi/2],
        "7": [-math.pi/2, -math.pi/4],
        "8": [-math.pi/4, 0]
    }
