    ...
    for item in data:
        ...
        if item['state'] == 'Pressed':
            if len(new_data) > SystemEnv.MIN_ACTION_LENGTH:
                ...
                feature = stats_action_feature(new_data, start, n_to, SystemEnv.PC_CODE, action_file)
                if SystemEnv.DEBUG:
                    print("PC", new_data)
            return
        else:
            if item['t'] - prev_time > SystemEnv.EXPIRATION_ACTION_TIME:
                ...
                if len(new_data) > SystemEnv.MIN_ACTION_LENGTH:
                    feature = stats_action_feature(new_data, start, stop, SystemEnv.MM_CODE, action_file)
                    if SystemEnv.DEBUG:
                        print("MM", new_data)
                new_data = []
                start = stop + 1
...
def process_move_actions(data, action_file, n_from, n_to):
    ...
    if len(new_data) > SystemEnv.MIN_ACTION_LENGTH:
        feature = stats_action_feature(new_data, start, n_to, SystemEnv.MM_CODE, action_file)
        if SystemEnv.DEBUG:
            print("MM", new_data)
    return
...
def process_drag_actions(data, action_file, n_from, n_to):
    ...
    for item in data:
        ...
        if item['button'] == 'Left' and item['state'] == 'Released':
            # ends the DD action
            new_data.append(item)
            feature = stats_action_feature(new_data, start, n_to, SystemEnv.DD_CODE, action_file)
            if SystemEnv.DEBUG:
                print("DD", new_data)
